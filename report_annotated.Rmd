...
source(here::here("00_fun_data_helpers.R"))
...
Below we can see the observed values of ALT vs triglycerides in the NHANES data 2013-2014. There are `r nrow(mydata)` subjects in this year of data, and `r sum(mydata$cvd_nohf, na.rm=T)` with CVD (as defined). We must use survey design methods to estimate the prevalence and quantiles of variables of interest within the full US population, which the survey sample is designed to represent.
...
In the total adult population, `r round(100*svy_mean_cvd[1],1)`% of adults in the US fulfill our definition of CVD. This equates to `r sprintf("%.0f",svy_mean_total[1])` people in the US.
...
So, we have our answer. Based on the 2013-2014 NHANES data, we estimate that approximately `r round(100*myprop,1)`% of patients have ALT < 60 in our sub-population of interest (CVD, no HF, trigs between 200-500).
...
We have a larger SE (`r round(myse2,3)`) using "regular" methods compared to the survey weighted SE (`r round(myse,3)`), so we gained precision by using the survey weighted methods, and also we have removed some bias from the estimate since we used the proper weights. 
